"""Sim 17 — Power dissipation by labelled domain vs frequency.

Computes the absorbed THz power in each of the five labelled domains
(axon, myelin_proximal, node, myelin_distal, external) as a function
of frequency by integrating ``ewfd.Qe`` (the EM power loss density,
W/m³) over each domain. This is the right observable for THz
absorption — what an experiment actually measures — rather than the
spot-sampled |E|² used in earlier sims.

The σ-encoding fix (now folded into ``apply_materials`` via
``freq_hz=...``) means the node's σ-loss is properly included.

Run: ``uv run python experiments/sim17_power_dissipation.py``
~5 minutes for 13 frequencies.
"""
...
from thznerve.model.geometry import (
    DOMAIN_LABELS, GeometryParams, build_geometry, selection_tag,
)
...
OUT_DIR = REPO_ROOT / "results" / "sim17"
...
MESH = MeshParams()
SIGMA_S_PER_M = 1.0  # active node-conductivity scenario
...
def _integrate_qe_per_domain(model, labels=DOMAIN_LABELS):
    """Integrate ewfd.Qe over each labelled domain. Returns dict label → power (W)."""
    java = model.java
    ...
    for label in labels:
        ...
        iv.set("data", "dset1")  # default dataset
        iv.set("expr", ["ewfd.Qe"])
        iv.selection().named(selection_tag(label))
        result = iv.computeResult()
        # Expect double[2][1][1] — single complex scalar
        try:
...
def main() -> None:
    ...
    for i, f_thz in enumerate(FREQ_THZ, start=1):
        ...
        model = client.create(f"sim17_freq_{i:02d}")
        build_geometry(model, GEOM)
        # Use the σ-aware material encoding at this frequency.
        apply_materials(
        ...
        row = [f_thz, powers.get("axon", 0.0), powers.get("myelin_proximal", 0.0),
               powers.get("node", 0.0), powers.get("myelin_distal", 0.0),
               powers.get("external", 0.0), total, dt]
        rows.append(row)
        print(f"  [{i:>2}/{len(FREQ_THZ)}] f={f_thz:5.3f} THz  "
              f"P_node={powers.get('node', float('nan')):.3e}  "
              f"P_water={powers.get('axon', float('nan')) + powers.get('external', float('nan')):.3e}  "
              f"P_total={total:.3e}  ({dt:4.1f}s)")
    ...
    ax_abs.plot(f_thz_arr, p_node, "-", color=OKABE_ITO[6], lw=1.6,
                label="node (with σ)")
    ...
    ax_rel.set_title("Node's share of absorption vs frequency")
    ax_rel.axvline(0.6, color="red", ls=":", lw=0.7, alpha=0.6)
    ax_rel.axvline(2.0, color="red", ls=":", lw=0.7, alpha=0.6)
    ax_rel.grid(True, alpha=0.3)

    fig.suptitle("Sim 17 — Per-domain power dissipation spectrum")
    fig.tight_layout()
    pdf, png = save_figure(fig, "sim17_power_dissipation")
    print(f"wrote {pdf}\nwrote {png}\nwrote {csv_path}")
